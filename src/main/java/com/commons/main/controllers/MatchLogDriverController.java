...
public class MatchLogDriverController {
	  ...
	  @GetMapping("api/v2/driver/match/history/{conversationId}")
	  public ResponseEntity<MatchLogHistory> getHistory(@RequestHeader Map<String, String> headers, @PathVariable(value="conversationId") String conversationId) throws Exception {
	    	log.debug("Header values: {}", headers);
	    	if(headers.get("api_secrete") == null || headers.get("api_version") == null || headers.get("api_deviceid") == null) {
	    		log.info("-----ValidationError : 412 Precondition failed");
	    		return ResponseEntity.status(412).body(null);
			  ...
			  MatchLogHistory history = driverImpl.getTripStatusHistory(conversationId);
			  if(history != null) {
